...
- A custom `name=` MUST be `<profile>-` followed by a NON-EMPTY suffix; a duplicate custom
  name is an error. (A named spawn surfaces the NAME, not the subagent type, to the
  contract hook — so the name must stay anchored to the Profile.)
