...
| `src/chinamaxM/` | the proxy package: `registry.py` (Registry loader), `proxy.py` (aiohttp app + router + Default branch), `relay.py` (relay mutation/serialize/forwarder), `keyfiles.py` (per-Host Key-file reader + O_EXCL scaffold), `data/profiles.json` (shipped seed Registry) |
| `tests/` | hermetic pytest suite: `conftest.py` (fake provider + guards + fixtures), `test_registry.py`, `test_proxy.py` (passthrough + routing + relay) |
