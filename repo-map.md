...
(proxy-01/02/03/04, hosts-01 generators, and hosts-02 Claude dispatch + worker-contract
hooks landed): the `docs/adr/` decisions of record plus the `src/`/`tests/` code and the
plugin `commands/`/`hooks/`/`scripts/` surfaces that realize them.
...
| `hooks/` | `hooks.json` — registers SubagentStart (no matcher) + PreToolUse(Agent), both → `scripts/worker_contract_hook` (additive; hosts-04 appends SessionStart) |
| `scripts/` | `worker_contract_hook` — executable stdin-buffering shim; pinned interpreter order (`CHINAMAXM_PYTHON` → setup-recorded path → `conda run` last resort) |
| `tests/` | hermetic pytest suite: `conftest.py` (fake provider + fake Codex client + guards + fixtures), `test_registry.py`, `test_proxy.py` (passthrough + routing + relay), `test_seam.py` (Responses ingress + Seam translation + streaming + startup gates), `test_observability_count_tokens.py` (JSONL line fields + usage tee + count_tokens forward/estimator/cache), `test_generate.py` (generation engine: Claude agents/Codex providers/roles + drift classification + set_model, against temp roots), `test_hosts_claude.py` (`/task` command-text invariants + hooks.json registration + LIVE worker-contract/Relay hook invocation) |
