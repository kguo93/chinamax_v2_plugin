...
| Path | What it is |
|---|---|
| `chinamaxM` | the CLI Launcher (executable): maps `{setup\|doctor\|set_model}` to its module, sources `_interpreter.sh`, and `chinamaxm_exec`s the module under the resolved interpreter; argv passes through verbatim after the subcommand. Every host command/skill surface runs plugin Python through this shim |
